...
export class WebFileSystem {
  ...
  async readFile(path, options = {}) {
    ...
    try {
      ...
      const file = await /** @type {FileSystemFileHandle} */ (
        fileHandle
      ).getFile();
      const encoding =
        (typeof options === "object" &&
          /** @type {{ encoding?: string }} */ (options).encoding) ||
  ...
  async stat(path) {
    ...
    try {
      ...
      const file =
        type === "file"
          ? await /** @type {FileSystemFileHandle} */ (handle).getFile()
          : null;
  ...
  async getFilesFromDirectory(path) {
    ...
    async function recursiveRead(dirHandle, currentPath) {
      for await (const entry of dirHandle.values()) {
        ...
        if (entry.kind === "file") {
          tasks.push(
            readFile(/** @type {FileSystemFileHandle} */ (entry), newPath),
          );
