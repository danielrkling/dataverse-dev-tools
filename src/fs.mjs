...
export class WebFileSystem {
     ...
     * @param {string} [dirName='web-fs'] Name of the root directory within OPFS.
     * @returns {Promise<WebFileSystem>}
     */
    static async fromOPFS(dirName = "web-fs") {
     ...
     * @returns {Promise<WebFileSystem>}
     */
    static async fromPicker(id = "web-fs") {
    ...
    async verifyPermission() {
        const currentStatus = await this.rootHandle.queryPermission({ mode: "readwrite" });
        if (currentStatus === "granted") {
            return true;
        }

        if (currentStatus === "prompt") {
            const newStatus = await this.rootHandle.requestPermission({ mode: "readwrite" });
    ...
    /**
     * Supported file read encodings.
     * @typedef {"utf8" | "utf-8" | "ascii" | "base64" | "latin1"} Encoding
     */

    /**
     * Options configuration for reading files.
     * @typedef {Object} ReadFileOptions
     * @property {Encoding} [encoding] The file encoding to use.
     */

    /**
     * Reads the content of a file.
     *
     * @template {ReadFileOptions | Encoding} [T={}]
     * @param {string} path The path of the file to read.
     * @param {T} [options={}] Options for reading the file (e.g., 'utf8' or { encoding: 'utf8' }).
     * @returns {Promise<T extends Encoding ? string : T extends { encoding: Encoding } ? string : ArrayBuffer>}
     *          The file content as a string if a valid encoding is specified, otherwise ArrayBuffer.
    ...
    async readFile(path, options = /** @type {any} */ ({})) {
        ...
        try {
            ...
            // Extract encoding from option string or option object
            const encoding = typeof options === "string" ? options : options?.encoding;

            // If a valid encoding is specified, return as string
            if (
                encoding === "utf8" ||
                encoding === "utf-8" ||
                encoding === "ascii" ||
                ...
                return /** @type {any} */ (await file.text());
            }

            // Default to ArrayBuffer
            return /** @type {any} */ (await file.arrayBuffer());
        } catch (e) {
