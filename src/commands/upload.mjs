...
const uploadParser = object({
  ...
  publish: flag("--publish", {
    description: message`Publish immediately after upload`,
  }),
});
...
export default createCommand({
  ...
  brief: "Upload web resources to Dataverse",
  execute: async (parsed, term) => {
    const { fs } = term;
    const rawPath = parsed.path;
    const publishImmediately = parsed.publish;
    ...
    try {
      ...
      if (stat.isDirectory) {
        ...
        const content = String(
          await fs.readFile(rawPath, { encoding: "utf8" }),
        );
    ...
    for (const [path, content] of files) {
      ...
      if (!isValidWebResource(name)) {
        const line = term.log(
          `${name} — <span style="color:#888">○ skipped (invalid web resource name)</span>`,
        );
        lines.set(path, line);
        continue;
      }
  ...
  init: async (term) => {
    ...
    async function flushPublish() {
      ...
      for (const item of batch) {
        ...
        if (line)
          line.innerHTML = `${item.name} — <span style="color:#569cd6">● publishing...</span>`;
      }
      ...
      try {
        ...
        term.dispatchEvent("publish:complete", {
          files: batch.map((i) => [i.path, i.content]),
        });
        if (upload.refresh === "onPublish")
          term.dispatchEvent("preview:refresh", {});
      } catch (e) {
        term.error(`Publish failed: ${e.message}`);
        term.dispatchEvent("publish:error", { error: e.message });
      }
    }
    ...
    const uploadFiles = async (filesToUpload) => {
      ...
      pm.emit("upload:complete", {
        files: filesToUpload,
        webResources: uploaded.map((i) => i.wr),
      });
